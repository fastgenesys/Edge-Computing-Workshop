...
def radar_with_circles_and_colors():
    ...
    # CHANGED: Using start_scan() for your RPLIDAR A2 hardware profile
    ...
    try:
        ...
        for scan in scan_generator():
            ...
            if prev_angle is not None and scan.angle < prev_angle:
                ...
                for r in range(500, MAX_DISTANCE + 1, 500):  # every 500 mm
                    pygame.draw.circle(screen, DARK_GREEN, CENTER, int(r * SCALE), 1)
                    
                    # CHANGED: Keeps the raw millimeter value 'r' and prints "mm"
                    label = font_small.render(f"{r} mm", True, WHITE)  
                ...
                for ang, dist in points:
                    ...
                    if dist <= 1000:       # 0–1000 mm
                        color = RED
                    elif dist <= 2000:     # 1000–2000 mm
                        color = YELLOW
                    else:                  # 2000–3000 mm
                        color = GREEN
